...
export default function TributeCategories() {
  ...
  const fetchList = async (p = 1) => {
    ...
    setLoading(true);
    try {
      const data = await getData<{ total: number; list: any[] }>(endpoints.sacrificeCategoryList, {
        ...
        <Space>
          <Button
            ...
            onClick={() => {
              ...
              editForm.setFieldsValue({ category_name: record.category_name });
              setOpen(true);
            }}
          >
            编辑
    ...
    >
      ...
      <Modal
        ...
        onOk={async () => {
          ...
          if (editingId) {
            await postData<any>(endpoints.sacrificeCategoryEdit, { id: editingId, ...v });
          } else {
            await postData<any>(endpoints.sacrificeCategoryAdd, v);
          }
          setOpen(false);
          await fetchList(1);
        }}
